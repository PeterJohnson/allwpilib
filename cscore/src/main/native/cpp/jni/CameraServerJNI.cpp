...
static jobject MakeJObject(JNIEnv* env, const cs::ServerConfig& config) {
  static jmethodID constructor =
      env->GetMethodID(serverConfigCls, "<init>",
                       ...
                       "[Ledu/wpi/cscore/VideoSource;)V");
  JLocal<jstring> address(env, MakeJString(env, config.address));
  JLocal<jobject> defaultSource(env,
                                MakeJVideoSource(env, config.defaultSource));
  JLocal<jobjectArray> onlySources(
      env, MakeJVideoSourceArray(env, config.onlySources));
 ...
 */
JNIEXPORT jint JNICALL
Java_edu_wpi_cscore_CameraServerJNI_getNodeProperty
  (JNIEnv* env, jclass, jint node, jstring name)
{
  if (!name) {
  ...
  CS_Status status = 0;
  auto val = cs::GetNodeProperty(node, JStringRef{env, name}.str(), &status);
  CheckStatus(env, status);
  return val;
}
...
/*
 * Class:     edu_wpi_cscore_CameraServerJNI
 * Method:    enumerateNodeProperties
 * Signature: (I)[I
 */
JNIEXPORT jintArray JNICALL
Java_edu_wpi_cscore_CameraServerJNI_enumerateNodeProperties
  (JNIEnv* env, jclass, jint node)
{
  CS_Status status = 0;
  wpi::SmallVector<CS_Property, 32> buf;
  auto arr = cs::EnumerateNodeProperties(node, buf, &status);
  if (!CheckStatus(env, status)) return nullptr;
...
/*
 * Class:     edu_wpi_cscore_CameraServerJNI
 * Method:    setNodeConfigJson
 * Signature: (ILjava/lang/String;)Z
 */
JNIEXPORT jboolean JNICALL
Java_edu_wpi_cscore_CameraServerJNI_setNodeConfigJson
  (JNIEnv* env, jclass, jint node, jstring config)
{
  CS_Status status = 0;
  auto val = cs::SetNodeConfigJson(node, JStringRef{env, config}, &status);
  CheckStatus(env, status);
  return val;
}
...
/*
 * Class:     edu_wpi_cscore_CameraServerJNI
 * Method:    getNodeConfigJson
 * Signature: (I)Ljava/lang/String;
 */
JNIEXPORT jstring JNICALL
Java_edu_wpi_cscore_CameraServerJNI_getNodeConfigJson
  (JNIEnv* env, jclass, jint node)
{
  CS_Status status = 0;
  auto val = cs::GetNodeConfigJson(node, &status);
  CheckStatus(env, status);
 ...
 */
JNIEXPORT jint JNICALL
Java_edu_wpi_cscore_CameraServerJNI_copyNode
  (JNIEnv* env, jclass, jint node)
{
  CS_Status status = 0;
  auto val = cs::CopyNode(node, &status);
  CheckStatus(env, status);
  return val;
}
 ...
 */
JNIEXPORT void JNICALL
Java_edu_wpi_cscore_CameraServerJNI_releaseNode
  (JNIEnv* env, jclass, jint node)
{
  CS_Status status = 0;
  cs::ReleaseNode(node, &status);
  CheckStatus(env, status);
}

/*
 * Class:     edu_wpi_cscore_CameraServerJNI
 * Method:    isNodeEnabled
 * Signature: (I)Z
 */
JNIEXPORT jboolean JNICALL
Java_edu_wpi_cscore_CameraServerJNI_isNodeEnabled
  (JNIEnv* env, jclass, jint node)
{
  CS_Status status = 0;
  auto val = cs::IsNodeEnabled(node, &status);
  CheckStatus(env, status);
  return val;
}
...
JNIEXPORT jint JNICALL Java_edu_wpi_cscore_CameraServerJNI_startServer
  (JNIEnv* env, jclass, jstring address, jint port, jint defaultSource, jintArray onlySources)
{
  ...
  jint* onlySourcesInts = env->GetIntArrayElements(onlySources, nullptr);
  if (!onlySourcesInts) return 0;
  size_t len = env->GetArrayLength(onlySources);
  config.onlySources.reserve(len);
  for (size_t i = 0; i < len; ++i)
    config.onlySources.emplace_back(onlySourcesInts[i]);
  env->ReleaseIntArrayElements(onlySources, onlySourcesInts, JNI_ABORT);
