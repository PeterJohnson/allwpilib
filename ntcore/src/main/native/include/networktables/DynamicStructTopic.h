...
class DynamicStructTopic;

/**
 * NetworkTables dynamic struct-encoded value subscriber.
 */
class DynamicStructSubscriber : public Subscriber {
 public:
  using TopicType = DynamicStructTopic;
  using ValueType = DynamicStruct;
  using ParamType = const T&;
  using TimestampedValueType = TimestampedDynamicStruct;

  DynamicStructSubscriber() = default;
   ...
   */
  explicit DynamicStructSubscriber(NT_Subscriber handle) : Subscriber{handle} {}
 ...
 private:
  const wpi::StructDescriptor* m_desc = nullptr;
};

/**
 * NetworkTables struct-encoded value publisher.
 */
class DynamicStructPublisher : public Publisher {
 public:
  using TopicType = DynamicStructTopic;
  using ValueType = T;
  using ParamType = const T&;
  ...
  DynamicStructPublisher() = default;

  DynamicStructPublisher(const DynamicStructPublisher&) = delete;
  DynamicStructPublisher& operator=(const DynamicStructPublisher&) = delete;
 ...
 */
class DynamicStructEntry final : public DynamicStructSubscriber,
                                 public DynamicStructPublisher {
 public:
  using SubscriberType = DynamicStructSubscriber;
  using PublisherType = DynamicStructPublisher;
  using TopicType = DynamicStructTopic;
  using ValueType = T;
  using ParamType = const T&;
   ...
   * @param info optional struct type info
   */
  explicit DynamicStructEntry(NT_Entry handle)
      : DynamicStructSubscriber{handle}, DynamicStructPublisher{handle} {}
   ...
   * @return Topic
   */
  TopicType GetTopic() const { return DynamicStructSubscriber::GetTopic(); }
 ...
 */
class DynamicStructTopic final : public Topic {
 public:
  using SubscriberType = DynamicStructSubscriber;
  using PublisherType = DynamicStructPublisher;
  using EntryType = DynamicStructEntry;
  using ValueType = T;
  using ParamType = const T&;
  using TimestampedValueType = TimestampedDynamicStruct;

  DynamicStructTopic() = default;
   ...
   * @param info optional struct type info
   */
  explicit DynamicStructTopic(NT_Topic handle) : Topic{handle} {}
   ...
   * @param info optional struct type info
   */
  explicit DynamicStructTopic(Topic topic) : Topic{topic} {}
  ...
  SubscriberType Subscribe(
    ...
    return DynamicStructSubscriber{::nt::Subscribe(
        m_handle, NT_RAW, wpi::GetStructTypeString(), options)};
  }
